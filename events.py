...
enemySpawn = {EVENT_FODDER: [], EVENT_SINE: [], EVENT_GRUNT : [], EVENT_SENTRY : []}
...
def addEventSinewave(size):
	...
	x = random.randint(entity.RSINE + entity.SINEAMP, size[0] - entity.RSINE - entity.SINEAMP)
...
def addEvents(size, difficulty):
	...
	if random.random() < fiftheenth : addEventScope(size)
	if random.random() < fiftheenth : addEventAS(size)
	if random.random() < fiftheenth * difficulty: addEventHPup(size)
